...
(function (trv, $, window, document, undefined) {
    ...
    function SendEmail(placeholder, options, otherOptions) {
        ...
        var controller = options.controller, initialized = false, viewerOptions = otherOptions, dialogVisible = false,
            $placeholder, $sendFormPlaceholder, kendoSendEmailDialog, inputFrom, inputTo, inputCC, inputSubject,
            docFormat, docFormatInput, bodyEditor, bodyText, docFormatList, optionsCommandSet, windowLocation;
        if (!controller) {
        ...
        function adjustDialogPosition() {
            if (!windowLocation) {
                ...
                var padding = 10, kendoWindow = kendoSendEmailDialog.element.parent(".k-window"),
                    left = windowLocation.left, top = windowLocation.top, width = kendoWindow.outerWidth(true),
                    height = kendoWindow.outerHeight(true), right = left + width, bottom = top + height,
        ...
        function validateFields() {
            var fromIsValid = isEmpty(inputFrom) || !isValidEmail(inputFrom),
                toIsValid = isEmpty(inputTo) || !isValidEmail(inputTo),
                ccIsValid = inputCC.val().length && !isValidEmail(inputCC), hasFormat = docFormat.value().length;
            if (!hasFormat) {
        ...
        function replaceStringResources($sendEmailDialog) {
            ...
            var labels = $sendEmailDialog.find(".trv-replace-string"),
                ariaLabel = $sendEmailDialog.find("[aria-label]"), titles = $sendEmailDialog.find("[title]");
            if (labels.length) {
...
(function (trv, $, undefined) {
    ...
    var JSON_MIME_TYPE = "application/json", JSON_CONTENT_TYPE = "application/json; charset=UTF-8",
        URLENCODED_CONTENT_TYPE = "application/x-www-form-urlencoded; charset=UTF-8", HTTP_GET = "GET",
        HTTP_POST = "POST", HTTP_PUT = "PUT", HTTP_DELETE = "DELETE";
    var defaultOptions = {};
...
(function (trv, window, document, undefined) {
    ...
    function ReportViewerController(options) {
        var controller = {}, clientId, reportInstanceId, reportDocumentId, registerClientPromise,
            registerInstancePromise, documentFormatsPromise, report, parameterValues, currentPageNumber, pageCount,
            viewMode = trv.ViewModes.INTERACTIVE, loader, printMode = trv.PrintModes.AUTO_SELECT, bookmarkNodes,
            clientHasExpired = false, parameterValidators = trv.parameterValidators, events = new Events();
        clearReportState();
        options = utils.extend({}, defaultOptions, options);
...
(function (trv, $, window, document, undefined) {
    ...
    function PagesArea(placeholder, options, otherOptions) {
        ...
        var $placeholder = $(placeholder), $pageContainer = $placeholder.find(".trv-page-container"),
            pageContainer = $pageContainer[0], $pageWrapper = $placeholder.find(".trv-page-wrapper"),
            pageWrapper = $pageWrapper[0], $errorMessage = $placeholder.find(".trv-error-message"), actions,
            pendingElement, pageScaleMode = scaleModes.SPECIFIC, pageScale = 1, minPageScale = .1, maxPageScale = 8,
            documentReady = true, navigateToPageOnDocReady, navigateToElementOnDocReady, isNewReportSource,
            showErrorTimeoutId;
        init();
        ...
        function setPageDimensions(page, scaleMode, scale) {
            var $target = $(page), $page = $target.find("div.trv-report-page"),
                $pageContent = $target.find("div.sheet"), pageContent = $pageContent[0];
            if (!pageContent) return;
            var pageWidth, pageHeight, box = $target.data("box");
            if (!box) {
                var margins = domUtils.getMargins($target), borders = domUtils.getBorderWidth($page),
        ...
        function setPageContent(page) {
            ...
            var wrapper = $($.parseHTML(page.pageContent)), $pageContent = wrapper.find("div.sheet"),
...
(function (trv, $, window, document, undefined) {
    ...
    trv.parameterEditors = [{
        ...
        createEditor: function (placeholder, options) {
            ...
            var $list = $placeholder.find(".trv-list"), $selectAll = $placeholder.find(".trv-select-all"),
                $selectNone = $placeholder.find(".trv-select-none"), listView, parameter, updateTimeout,
                valueChangeCallback = options.parameterChanged, initialized;
        ...
        createEditor: function (placeholder, options) {
            ...
            var $list = $placeholder.find(".trv-list"), $selectNone = $placeholder.find(".trv-select-none"), listView,
                parameter, valueChangeCallback = options.parameterChanged;
            if ($selectNone) {
                $selectNone.text(sr[$selectNone.text()]);
                $selectNone.click(function (e) {
                    e.preventDefault();
...
(function (trv, $, window, document, undefined) {
    ...
    function ParametersArea(placeholder, options, otherOptions) {
        ...
        var $placeholder = $(placeholder), $content = $placeholder.find(".trv-parameters-area-content"),
            $errorMessage = $placeholder.find(".trv-error-message"),
            $previewButton = $placeholder.find(".trv-parameters-area-preview-button"),
            noParametersContent = $placeholder.html();
        ...
        function createParameterUI(parameter) {
            var $container = createParameterContainer(), $editorPlaceholder = $container.find(".trv-parameter-value"),
                $title = $container.find(".trv-parameter-title"), $error = $container.find(".trv-parameter-error"),
                $errorMessage = $container.find(".trv-parameter-error-message"),
                $useDefaultValueCheckbox = $container.find(".trv-parameter-use-default input"),
...
(function (trv, $, window, document, undefined) {
    ...
    function uiController(options) {
        ...
        var controller = options.controller, historyManager = options.history, state = 0, refreshUI,
...
(function (trv, $, window, document, undefined) {
    ...
    function MainMenu(dom, options, otherOptions) {
        ...
        var menu = $(dom).data("kendoMenu"), childrenL1 = dom.childNodes, controller = options.controller,
            enableAccessibility = options.enableAccessibility, DEFAULT_TABINDEX = 1;
        if (!menu) {
            init();
        }
...
(function (trv, $, window, document, undefined) {
    ...
    function attachCommands($element, args) {
        var commands = args[0].commands, viewerOptions = args[1],
            elementSelector = '[data-command^="telerik_ReportViewer_"]',
            customElementSelector = "[data-target-report-viewer]" + elementSelector;
        ...
        function customCommandHandler(e) {
            var $this = $(this), prefixedDataCommand = $this.attr("data-command"),
                reportViewerTarget = $this.attr("data-target-report-viewer");
            if (prefixedDataCommand && reportViewerTarget) {
                var dataCommand = prefixedDataCommand.substring("telerik_ReportViewer_".length),
                    reportViewer = $(reportViewerTarget).data("telerik_ReportViewer"),
                    cmd = reportViewer.commands[dataCommand];
                if (cmd.enabled()) {
                    cmd.exec($(this).attr("data-command-parameter"));
                }
                e.preventDefault();
    ...
    function attachCommand(dataCommand, cmd, viewerOptions, $element) {
        if (cmd) {
            var elementSelector = '[data-command="telerik_ReportViewer_' + dataCommand + '"]',
                customElementSelector = '[data-target-report-viewer="' + viewerOptions.selector + '"]' + elementSelector,
                $defaultElement = $element.find(elementSelector), $customElement = $(customElementSelector);
            ...
            utils.each(menuAreas, function () {
                ...
                utils.each(menuItems, function () {
                    ...
                    if (!$menuItem.hasClass("trv-report-pager")) {
                        ...
                        var expandableSr = utils.stringFormat(". {0}", [sr.ariaLabelExpandable]),
                            expandable = $menuItem.find("ul").length > 0 && ariaLabel.indexOf(expandableSr) < 0 ? expandableSr : "";
                        var selectedSr = utils.stringFormat(". {0}", [sr.ariaLabelSelected]),
                            selected = $menuItem.hasClass("k-state-selected") && ariaLabel.indexOf(selectedSr) < 0 ? selectedSr : "";
...
(function (trv, $, window, document, undefined) {
    ...
    function Search(placeholder, options, otherOptions) {
        options = $.extend({}, defaultOptions, options);
        var controller = options.controller, initialized = false, viewerOptions = otherOptions, dialogVisible = false,
            $placeholder, $inputBox, $searchOptionsPlaceholder, $resultsLabel, $resultsPlaceholder, kendoComboBox,
            kendoSearchDialog, stopSearchCommand, optionsCommandSet, navigationCommandSet, searchResults, mruList = [],
            inputComboRebinding, searchMetadataRequested, searchMetadataLoaded, pendingHighlightItem, windowLocation;
        ...
        function adjustDialogPosition() {
            if (!windowLocation) {
                ...
                var padding = 10, kendoWindow = kendoSearchDialog.element.parent(".k-window"),
                    left = windowLocation.left, top = windowLocation.top, width = kendoWindow.outerWidth(true),
                    height = kendoWindow.outerHeight(true), right = left + width, bottom = top + height,
                    windowWidth = $(window).innerWidth(), windowHeight = $(window).innerHeight();
                if (right > windowWidth - padding) {
        ...
        function replaceStringResources($search) {
            ...
            var $searchCaption = $search.find(".trv-search-dialog-caption-label"),
                $searchOptions = $search.find(".trv-search-dialog-search-options"),
                $searchStopButton = $search.find("a[data-command='telerik_ReportViewer_searchDialog_StopSearch']"),
                $searchMatchCaseButton = $search.find("a[data-command='telerik_ReportViewer_searchDialog_MatchCase']"),
                $searchMatchWholeWordButton = $search.find("a[data-command='telerik_ReportViewer_searchDialog_MatchWholeWord']"),
                $searchUseRegexButton = $search.find("a[data-command='telerik_ReportViewer_searchDialog_UseRegex']"),
                $searchNavigateUpButton = $search.find("a[data-command='telerik_ReportViewer_searchDialog_NavigateUp']"),
...
(function (trv, $, window, document, undefined) {
    ...
    function ReportViewer(dom, options) {
        ...
        var $placeholder = $(dom), templates = {}, scripts = {},
            persistanceKey = options.id || "#" + $placeholder.attr("id"), accessibility;
